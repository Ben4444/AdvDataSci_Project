...
packages<-c("tibble","dplyr", "tidyr", "stringr", "jsonlite", "rdrop2", "data.table", "ggplot2", "maps")
...
# Loading in Texas and Louisiana state maps
states <- map_data("state")
states <- subset(states, region=="texas" | region=="louisiana")
```
...
# Plotting all observations together
alltweets <- ggplot(states)
alltweets <- alltweets + geom_path(aes(x=long, y=lat, group=group), color=gray(2/3), lwd=1/3)
alltweets <- alltweets + geom_point(data = final, aes(x=longitude, y=latitude), color="RED", alpha=1/2, size=1)
alltweets <- alltweets + coord_equal()
