...
1) The first step is to gain access to the Twitter API, which I have done by following this [tutorial](http://adilmoujahid.com/posts/2014/07/twitter-analytics/). Here is the link to my [Twitter app page](https://apps.twitter.com/app/14193250).

2) Next, I need to download and install the Python library Tweepy, in order to connect to Twitter Streaming API and download data. I have done this by following these [installation instructions](https://github.com/tweepy/tweepy). Note: Python and pip must be installed first (For pip: Must type 'python -m pip XXXX' (without quotes) in order for the command to work).

3) Next, I created a Python program in Notepad++ called twitter_streaming.py, based off of these [instructions](http://www.mikaelbrunila.fi/2017/03/27/scraping-extracting-mapping-geodata-twitter/), and have started my stream searching for the hashtags 'HurricaneHarvey' and 'HurricaneHarveyRelief'. I am exporting the output to a JSON file, twitter_data.json, which I will later have to parse for relevant data.
...
5) I stopped the Twitter data stream at 9:56AM on 9/4/2017. Program run time = 36 hours, 46 minutes. A total of 3,289,336 KB (3.290 GB) of Twitter data was collected, which corresponds to 1,491.086 KB of data per minute, on average.


**Analyzing Data**

I next created another Python program in Notepad++ called tweet_geolocator.py, again based off of these [instructions](http://www.mikaelbrunila.fi/2017/03/27/scraping-extracting-mapping-geodata-twitter/). 
Note: Try importing coordinates into R - may first have to pull out in Python. Then import JSON file into R (jsonlite) and then map (ggplot) and cut out all non-relevant locations (Tweets in the U.S. for identifying locations)
...
3) After the tweets are geocoded and restricted, I will import them into Leaflet? for visualization.
